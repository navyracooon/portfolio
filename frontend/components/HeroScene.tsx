...
function IslandField({
  ...
  return (
    <group position={[0, -1.0, 0]}>
      {islands.map((island) => (
        <IslandModel key={island.id} island={island} active={selected?.id === island.id} onSelect={onSelect} />
      ))}
    </group>
  );
}

function SelectedPanel({ selected, onClose }: { selected: IslandConfig | null; onClose: () => void }) {
  if (!selected) {
    return (
