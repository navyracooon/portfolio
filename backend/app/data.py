from app.schemas import PortfolioPayload, Profile, Project

PROFILE = Profile(
    name="Navy Racooon",
    role="Creative Developer / Full-Stack Engineer",
    intro=(
        "Three.js を単なる装飾ではなく、各詳細ページへの入口として扱うポートフォリオ。"
        "クリック操作は FastAPI に記録しつつ、行動データで導線そのものは歪めない設計にしています。"
    ),
...
PROJECTS = [
    Project(
        ...
        impact=(
            "教育用途の画面でも情報量を整理し、学生が必要な導線へ素早く到達できる UI を目指した。"
        ),
        overview=(
            "大学 LMS をモバイルから扱いやすくするためのクライアントアプリ想定。課題、通知、"
            "コース導線を整理し、複雑な情報構造を日常的な操作に落とし込むことを重視した。"
        ),
    ...
    Project(
        ...
        impact=(
            "フロントエンド表現、イベント記録、公開 API、運用設計を一つの体験として"
            "説明できる構成に整理。"
        ),
        overview=(
            "固定配置の 3D オブジェクトをプロジェクト・問い合わせの入口として使い、操作イベントは"
            "バックエンドに記録する。一方で、行動データによるランキングやサイズ変更は行わず、"
            "情報設計の主導権を UI 側に残す。"
        ),
    ...
    Project(
        ...
        overview=(
            "数理最適化、制御、シミュレーションの知見を、Web アプリや運用設計と切り離さずに"
            "示すためのプロジェクト。研究内容そのものより、問題設定と実装思考を伝えることを重視している。"
        ),
    ...
    Project(
        ...
        overview=(
            "Neovim、zsh、tmux などの設定をまとめたリポジトリ。派手な成果物ではないが、"
            "毎日の操作効率や思考速度に直接効く領域として扱っている。"
        ),
    ...
    Project(
        ...
        overview=(
            "Raspberry Pi 上で Caddy、Docker Compose、Cloudflare DNS を使い、公開用サービスと"
            "内部サービスを分離して運用する構成。Headscale の詳細そのものではなく、"
            "境界設計と公開範囲の制御を主題にしている。"
        ),
